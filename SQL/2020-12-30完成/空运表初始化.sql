...
CREATE TABLE `air_booking` (
  ...
  `air_order_no` varchar(50) NOT NULL COMMENT '空运订单编号',
  ...
  `delivery_warehouse` varchar(200) DEFAULT NULL COMMENT '交仓仓库',
  `file_path` varchar(500) DEFAULT NULL COMMENT '提单文件路径(多个逗号隔开)',
  `file_name` varchar(500) DEFAULT NULL COMMENT '提单文件名称(多个逗号隔开)',
  `create_user` varchar(50) DEFAULT NULL COMMENT '创建人(登录用户)',
  `create_time` datetime DEFAULT NULL COMMENT '创建时间',
  `update_user` varchar(50) DEFAULT NULL COMMENT '更新人',
  `update_time` datetime DEFAULT NULL COMMENT '更新时间',
  `bill_lading_weight` double DEFAULT NULL COMMENT '提单重量',
  `billing_weight` double DEFAULT NULL COMMENT '计费重量',
  PRIMARY KEY (`id`) USING BTREE
) ENGINE=InnoDB AUTO_INCREMENT=21 DEFAULT CHARSET=utf8 ROW_FORMAT=DYNAMIC COMMENT='空运订舱表';
...
CREATE TABLE `air_order` (
  ...
  `order_no` varchar(200) DEFAULT NULL COMMENT '空运订单编号',
  `third_party_order_no` varchar(200) DEFAULT NULL COMMENT '第三方订单编号',
  `status` char(10) DEFAULT NULL COMMENT '状态(A_0待接单,A_1空运接单,A_2订舱,A_3订单入仓,\r\nA_4确认提单,A_5确认离港,A_6确认到港,A_7海外代理A_8确认签收)',
  `process_status` int(10) DEFAULT NULL COMMENT '流程状态(0:进行中,1:完成,2:草稿,3.关闭)',
  `settlement_unit_code` char(20) DEFAULT NULL COMMENT '结算单位code',
...
CREATE TABLE `air_port` (
  `id` bigint(20) NOT NULL AUTO_INCREMENT COMMENT '主键',
  `code` varchar(50)  DEFAULT NULL COMMENT '飞机港口代码',
  `name` varchar(50) DEFAULT NULL COMMENT '飞机港口名称',
...
CREATE TABLE `goods` (
  ...
  `business_type` int(10) NOT NULL COMMENT '业务类型(0:空运;1:服务单)',
  `label` varchar(50) DEFAULT NULL COMMENT '标签（空运是唛头）',
  `name` varchar(600) DEFAULT NULL COMMENT '货品名称',
  `plate_amount` int(10) DEFAULT NULL COMMENT '板数',
  `plate_unit` varchar(20) DEFAULT NULL COMMENT '板数单位',
  `bulk_cargo_amount` int(10) DEFAULT NULL COMMENT '散货件数',
  `bulk_cargo_unit` varchar(20) DEFAULT NULL COMMENT '散货单位',
  `size` varchar(1000) DEFAULT NULL COMMENT '尺寸(长宽高)',
  `total_weight` double(20,2) DEFAULT NULL COMMENT '总重量',
  `volume` double(20,2) DEFAULT NULL COMMENT '体积',
  `create_time` datetime DEFAULT NULL COMMENT '创建时间',
  PRIMARY KEY (`id`) USING BTREE
) ENGINE=InnoDB AUTO_INCREMENT=19 DEFAULT CHARSET=utf8 ROW_FORMAT=DYNAMIC COMMENT='货品信息表';
...
CREATE TABLE `order_address` (
  ...
  PRIMARY KEY (`id`) USING BTREE
) ENGINE=InnoDB AUTO_INCREMENT=39 DEFAULT CHARSET=utf8 ROW_FORMAT=DYNAMIC COMMENT='订单地址表';
