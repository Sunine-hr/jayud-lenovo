 ...
 */
@Service
public class PaySupplierBillTemplateServiceImpl implements BillTemplateService {
...
//        titles.add("客户应付款对帐单");
        ...
        stageHeads.add("TO:" + viewBillVO.getLegalName() + settlementRelationship.get(1));
        sb = new StringBuilder();
        stageHeads.add(sb.append("FR:").append(viewBillVO.getCustomerName()).append(settlementRelationship.get(0))
    ...
    private void settlementTaxRate(String cmd, String templateCmd, String billNo,
                                   LinkedHashMap<String, String> headMap,
                                   LinkedHashMap<String, String> dynamicHead,
                                   JSONArray datas) {

        //结算汇率
        if (templateCmd.equals(BillTemplateEnum.ZGYS_SUPPLIER_PAY.getCmd())) {

            String settlementCurrency = datas.getJSONObject(0).getStr("settlementCurrency");

            String originalCurrency;
            //结算币种港币->人民币汇率,人民币->港币汇率
            ...
            List<OrderBillCostTotal> list = this.orderBillCostTotalService.getByCondition(new OrderBillCostTotal().setBillNo(billNo)
                    .setCurrencyCode(settlementCurrency).setCurrentCurrencyCode(originalCurrency).setMoneyType("1"));

            if (CollectionUtils.isEmpty(list)) {
                return;
            }

            String key = "exchangeRate";
            String head = "汇率";
            dynamicHead.put(key, head);
            headMap.put(key, head);
            BigDecimal exchangeRate = list.get(0).getExchangeRate();
            for (int i = 0; i < datas.size(); i++) {
                JSONObject object = datas.getJSONObject(i);
                object.put(key, exchangeRate);
//            }
            }
        }
    }
