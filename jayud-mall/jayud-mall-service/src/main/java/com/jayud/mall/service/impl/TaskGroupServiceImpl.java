    ...
    public CommonResult<TaskGroupVO> saveTaskGroup(TaskGroupForm form) {
        ...
        if(types == 1){
            ...
            billTasks.forEach(billTask -> {
                String taskCode = billTask.getTaskCode();
                TaskVO taskVO = taskMapper.findTaskByTaskCode(taskCode);
                String taskName = taskVO.getTaskName();
                billTask.setTaskName(taskName);
            ...
            waybillTasks.forEach(waybillTask -> {
                String taskCode = waybillTask.getTaskCode();
                TaskVO taskVO = taskMapper.findTaskByTaskCode(taskCode);
                String taskName = taskVO.getTaskName();
                waybillTask.setTaskName(taskName);
