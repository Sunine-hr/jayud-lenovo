...
public class OauthTask {
     ...
     * 同步消息渠道手机号码
     */
    @XxlJob("syncChannelMobileAccount")
    public void syncChannelMobileAccount() throws Exception {
        XxlJobHelper.log("执行同步消息渠道手机号码任务");
        log.info("执行同步消息渠道手机号码任务");
        ApiResult result = oauthClient.syncChannelMobileAccount();
        if (!result.isOk()) {
